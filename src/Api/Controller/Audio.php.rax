    ...
    public static function speech_to_text(App $object){
        try {
            ...
            if($role){
                ...
                //need host backend
                $name = 'Domain:' . Controller::name("{{$options.backend.host}}:Controller:Audio") . '.speech.to.text.task';
                ddd($name);
...
//                echo Response::output($object, $response);
     ...
     /**
         ...
         * @throws ORMException
         */
        public static function speech_to_text_task(App $object){
            try {
                $role = Permission::controller(
                    $object,
                    Audio::NAME,
